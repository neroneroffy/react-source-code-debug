...
那个（些）优先级标记。完整代码可以看[这里]()，我做了一些注释

2. 开始工作循环
以root为起点，在后台构建workInProgress树。如下图。
...
在current树上采取深度优先的遍历策略构建WIP树，对于每个WIP节点来说，它会经历beginWork和completeWork的过程。而处理节点上的updateQueue就发生在
beginWork的过程中。

当一路向下遍历到产生更新的fiber节点时，以它为基础新建WIP节点。
```javascript
function performUnitOfWork(unitOfWork: Fiber): void {
  // unitOfWork就是当前遍历到的fiber节点
