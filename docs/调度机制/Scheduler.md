...
到此，我们梳理出Scheduler中两个重要的行为：**管理任务**、**单个任务的执行控制**
...
时间片规定的是单个任务在这一帧内最大的执行时间，保证页面不会因为任务执行时间过长而产生卡顿。
...
用户交互要通过占用线程去响应页面。真正通过Scheduler调度的React更新任务遇到用户交互之后，会是下面动图里的效果。

![](http://neroht.com/schedulerTask.gif)

任务执行和浏览器绘制页面响应用户交互这两件事情是互斥的关系，但因为Scheduler可以利用时间片中断更新任务，然后让出线程给浏览器的绘制，所以一开始在fiber树的构建阶段，
拖动方块会得到即使的反馈。后面卡了一下是因为fiber树构建完成，进入了同步的commit阶段，导致交互卡顿了一下。分析页面的渲染过程可以非常直观地看到通过时间片的控制。主线程被让出
去进行页面的绘制（Painting和Rendering，绿色和紫色的部分）。

![](http://neroht.com/schedulerTask.jpg)

Scheduler实现这样的效果需要两个角色：任务的调度者、任务的执行者。调度者调度一个执行者，执行者去循环taskQueue，逐个执行任务。当某个任务的执行时间比较长，
