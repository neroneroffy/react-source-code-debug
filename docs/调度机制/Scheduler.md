...
时间片规定的是单个任务在这一帧内最大的执行时间，任务一旦执行时间超过时间片，则会被打断，这样可以保证页面不会因为任务连续执行的时间过长而产生卡顿。

# 原理概述
基于任务优先级和时间片的概念，Scheduler围绕着它的核心目标 - 任务调度，衍生出了两大核心功能：任务队列管理 和 时间片下任务的中断以及恢复。
...
* timerQueue中，依据任务的开始时间（startTime）排序，开始时间越早，说明会越早开始，开始时间小的排在前面。任务进来的时候，开始时间默认是当前时间，如果调度的时候传了延迟时间，则是当前时间与延迟时间的和。

**任务入队两个队列，之后呢？**
如果放到了taskQueue，那么立即调度一个函数去循环taskQueue，挨个执行里面的任务。

如果放到了timerQueue，那么说明它里面的任务都不会立即执行，那就等到了它里面排在最前面的任务的开始时间，看任务是否过期，如果是，则把任务从timerQueue中拿出来放入taskQueue，
调度一个函数去循环它，执行掉里面的任务；否则过一会继续检查。
...
拖动方块会得到及时的反馈。但是后面卡了一下，不过这是因为fiber树构建完成，进入了同步的commit阶段，导致交互卡顿。分析页面的渲染过程可以非常直观地看到通过时间片的控制。主线程被让出
...
重复这一过程，直到当前这个任务彻底完成后，将任务出队。taskQueue中每一个任务都被这样处理，最终完成所有任务，这就是Scheduler的完整工作流程。
...
意味着它将会是立即过期的任务，会通过调度宏任务立即执行掉它，这样能够保证在下一次事件循环中执行掉任务。
```javascript
...
等到了timerQueue中排在最前面的任务的开始时间点，再去检查它是否过期，如果过期则放到taskQueue中，任务就可以被执行了。否则继续等。这个过程通过`handleTimeout`完成。
...
```
它既然把`flushWork`作为入参，那么任务的**执行者**本质上调用的就是`flushWork`.我们先不管执行者是如何执行任务的，先关注执行者是如何被调度的，
这需要看一下`requestHostCallback`的实现，找出**调度者**。
...
```
之所以有两种实现，是因为非浏览器环境不存在屏幕刷新率，没有帧的概念，也就不会有时间片，它们在执行任务的时候有本质区别，因为非浏览器环境基本不胡有用户交互，所以该场景下不判断任务执行时间是否
...
这个例子只保留了workLoop中判断任务完成状态的逻辑，其余的地方有并不完善，要以真正的的workLoop为准，现在让我们贴出它的全部代码，完整地看一下真正的实现：
```javascript
function workLoop(hasTimeRemaining, initialTime) {
...
那么会将返回的这个function再次赋值给任务的callback，由于任务还未完成，所以并不会被剔除出taskQueue，currentTask获取到的还是它，所以while循环到下一次还是会继续执行这个任务，
直到任务完成出队，才会继续下一个。
