...
## 事件监听器listener做了什么
一句话：负责以不同的优先级权重来触发真正的事件流程。

比如一个元素绑定了onClick事件，那么点击它的时候，绑定在root上的listener会被触发，它会**合成并分发事件对象**，**收集执行事件**。
...
**事件对象的合成** 、**将事件处理函数收集到执行路径**、 **事件执行**。
这个过程在源码中体现在`dispatchEventsForPlugins`函数中，函数体可看成两部分：**事件对象的合成和事件收集** 、 **事件执行**
```javascript
...
listeners是事件触发路径，event是合成事件对象，下面的内容说明了这两个过程。
...
### 模拟冒泡和捕获
冒泡和捕获的执行顺序是不一样的，但是当初在收集事件的时候，无论是冒泡事件还是捕获事件，都是直接push到路径里的。那么执行顺序的差异是如何体现的呢？
答案是循环路径的顺序不一样。

首先看一下dispatchQueue中的listeners中的事件处理函数排列顺序：触发事件的目标元素的事件处理函数排在第一个，上层组件的事件处理函数依次往后排。
```
...
```
liseners: [ onClickChild, onClickParent ]
