...
调用ensureListeningTo去绑定事件。入参中有一个`rootContainerElement`，它便是root节点，最终事件会绑定到它上面。

# 事件对象的合成和分发
在事件中拿到的事件对象不是原生的事件对象，而是经过React包装过的`SyntheticEvent`对象。在浏览器的环境中，若父子元素绑定了相同类型的事件，
