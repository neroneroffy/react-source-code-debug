...
```
registrationNameDependencies是一个对象，存储了所有React事件对应的原生DOM事件的集合，如果是事件类型的prop，那么将会调用ensureListeningTo去绑定事件。
接下来的绑定过程可以概括为如下几个关键点：
* 根据React的事件名称寻找该事件依赖，例如onMouseEnter事件依赖了mouseout和mouseover两个原生事件，onClick只以来了click一个原生事件。
* 识别该事件是否是捕获阶段的事件，例如`onClickCapture`这样的React事件名称
* 根据找出的原生事件，和是否是捕获阶段的事件，将事件绑定到root元素上
* 若事件需要更新，先移除事件监听，再重新绑定
...
在最开始的时候提到过，事件注册的过程决定了事件的优先级，事件的优先级则是根据事件的交互程度进一步划分的。优先级和事件的映射关系存在于一个Map结构中。

![事件优先级的Map](http://neroht.com/eventPriorities.jpg)

`createEventListenerWrapperWithPriority`函数会根据这些映射关系，生成三类不同优先级的事件监听。关于事件优先级的详细介绍请移步[React中的优先级](https://github.com/neroneroffy/react-source-code-debug/blob/master/docs/%E4%BA%8B%E4%BB%B6%E7%B3%BB%E7%BB%9F/React%E4%B8%AD%E7%9A%84%E4%BC%98%E5%85%88%E7%BA%A7.md)
值得注意的是，这里生成的持有不同优先级的事件监听，并不是我们写在组件中的事件处理函数。这个事件监听只负责以不同的优先级权重来派发事件，它会触发事件对象的合成和分发机制，进而让组件中真正的事件处理函数得以被触发。
也就是说绑定到root上的事件监听只是相当于一个传令官，它按照事件的优先级去安排接下来的工作：**事件对象的合成和分发** 和 **事件执行**，使得scheduler得以获知当前任务的优先级来展开调度。

# 事件对象的合成和分发
我们在事件处理函数中拿到的事件对象不是原生的事件对象，而是经过React包装过的`SyntheticEvent`对象。在浏览器的环境中，若父子元素绑定了相同类型的事件，
