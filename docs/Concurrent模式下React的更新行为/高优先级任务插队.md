...
让我们看一看`ensureRootIsScheduled`函数做的事情，先是准备本次任务调度协调所需要的数据，以及判断是否真的需要调度
* 获取root.callbackNode，即旧任务
* 整理一下root.pendingLanes，将过期任务放入root.expiredLanes，目的是让过期任务能够以同步优先级去进入调度（立即执行）
* 获取renderLanes，如果renderLanes是空的，说明不需要调度，直接return掉
...
* 同步优先级：传统的React同步渲染模式下的调度。通过React提供的`scheduleSyncCallback`函数将任务函数**performSyncWorkOnRoot**加入到React自己的同步
...
function ensureRootIsScheduled(root: FiberRoot, currentTime: number) {
  ...
  // 检查是否有过期任务，有则立即将它放到root.expiredLanes，
  // 便于接下来将这个任务以同步模式立即调度
...
对于不同优先级update的取舍以及root上pendingLanes的标记操作，这需要我们下沉到执行更新任务的过程中。
...
过程比较简单而且好理解，就是跳过update并且将它的lane放到fiber.lanes中。我们直接来看一下实现：
...
这是React处理更新时对优先级不足的update的处理，对于循环updateQueue，处理更新的过程实际上较为复杂，因为低优先级update会被跳过并且会重做，
所以这涉及到最终状态统一的问题，关于这一过程的原理解读在我的这篇文章里：[React中的状态计算]()，在本篇文章中只关注优先级相关的部分。

现在，被跳过的update的lane又被放入了fiber.lanes，现在，只需要将它放到root.pendingLanes中就能表示在本轮更新后，仍然有任务未被处理。
接下来的过程就是在completeWork阶段去收集这些lanes。

# 收集未被处理的lane
在completeUnitOfWork的时候，fiber.lanes再次被一层一层收集到父级fiber的childLanes中，最终，这些更新被作为remainLanes会复制给root.pendingLanes。
再次调用`ensureRootIsScheduled`，重做低优先级任务。
