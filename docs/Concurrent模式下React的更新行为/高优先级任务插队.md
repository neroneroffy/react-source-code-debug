...
## 任务优先级
任务本身是由更新产生的，因此任务优先级本质上是和update的优先级，即update.lane有关（只是有关，不一定是由它而来）。得出的任务优先级属于lanePriority，这与scheduler内部的调度
优先级是两个概念，React中的优先级转化关系可以看我总结过的一篇文章：[React中的优先级]()，我们这里只探讨任务优先级的生成过程。
...
目前为止我们了解了任务和任务优先级的本质，下面正式进入任务的调度过程。React这边对任务的调度本质上其实是以任务优先级为基准，对多个任务中的现有任务进行**或复用，或取消**的操作，
对单个任务进行**或同步，或异步，或批量同步（暂时不需要关注）** 的调度决策，这中行为可以看成是一种任务协调机制。
