...
```
对于上面的概念与源码的主体结构了解之后，放出完整代码，但删除了无关部分，我添加了的注释，对以上的过程都做了对应的标注，一定要抓住上边过程的重点，否则单看链表操作还是
有些复杂。
```
function processUpdateQueue<State>(
  workInProgress: Fiber,
...
# 总结
经过上面的梳理，可以看出来整个对更新的处理都是围绕优先级。整个processUpdateQueue函数要实现的目的是处理更新，但要保证更新按照优先级被处理的同时，不乱阵脚，是因为遵循一套固定的规则：优先级被跳过后，记住此时的状态和之后的更新队列并将队列备份
