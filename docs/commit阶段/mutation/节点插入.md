...
此时`<Child/>`的子节点已经全部完成插入，会再找到p的兄弟节点span，对它进行插入，然后发现span还有兄弟节点h1，将h1也插入。
...
```
在插入兄弟节点的时候，递归调用insertOrAppendPlacementNode的时候也传入了before，而这个before是最开始那个待插入的目标节点的基准节点，它有可能有，有可能没有。
